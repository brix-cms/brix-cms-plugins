...
public abstract class CreateTitledNodePanel extends BrixGenericPanel<BrixNode> {
    ...
    public CreateTitledNodePanel(String id, IModel<BrixNode> containerNodeModel, final String type, final SimpleCallback goBack,
    ...
    private void createNode(String type, String typeName) {
        ...
        if (parent.hasNode(name)) {
            String error = getString("resourceExists", new Model<CreateTitledNodePanel>(CreateTitledNodePanel.this),
                    "A " + typeName + " with that name already exists.  Please use a different name.");
            error(error);
        } else {
